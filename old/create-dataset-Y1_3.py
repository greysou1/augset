...
def read_mask_videos(video_paths, person_json_path):
    ...
    print(f"{len(json_data) = }, {len(clean_sil_indices) =  }")
    # print(clean_sil_indices)
    ...
    pe_masks = load_video(pe_sil_video, mask=True)
    sh_masks = load_video(sh_sil_video, mask=True)
    pa_masks = load_video(pa_sil_video, mask=True)

    print(f"{len(pe_masks) = }, {len(sh_masks) = }, {len(pa_masks) = }")
    i = 0
    while True:
        ...
        # print(f"{i = }, {int(keys[i]) in clean_sil_indices = }")
    ...
    print(f"{len(frames) = }, {len(clean_sil_indices) = }")
    cap_pe.release()
    cap_sh.release()
    cap_pa.release()
...
if __name__ == "__main__":
    ...
    save_root = "/home/c3-0/datasets/ID-Dataset/casiab/Y1_temp/"
    ...
    for video_file in t:
        ...
        # print(filename)
        # if sub_id != "007" or cond != 'bg-01' or view_angle != '072': continue
        if sub_id != "003" or cond != 'cl-01' or view_angle != '072': continue
