...
class SamPredictor:
    ...
    def set_torch_images(
        ...
        Arguments:
          transformed_images (torch.Tensor): The input images, with shape
            Bx3xHxW, which have been transformed with ResizeLongestSide.
          original_image_sizes (tuple(tuple(int, int))): The sizes of the images
            before transformation, in (H, W) format.
        ...
        ), f"set_torch_images input must be BCHW with long side {self.model.image_encoder.img_size}."
        self.reset_image()

        self.original_sizes = original_image_sizes
        self.input_sizes = tuple(transformed_images[0].shape[-2:])
        # self.input_sizes = [tuple(transformed_images[i].shape[-2:]) for i in range(batch_size)]
    ...
    def set_images(
        ...
        Arguments:
          images (np.ndarray): The images for calculating masks. Expects images
            in BHWC uint8 format, with pixel values in [0, 255].
        ...
        input_images = [self.transform.apply_image(image) for image in images]
        input_images_torch = torch.as_tensor(input_images, device=self.device)
        input_images_torch = input_images_torch.permute(0, 3, 1, 2).contiguous()

        self.set_torch_images(input_images_torch, images.shape[1:3], batch_size)
